...
class cell:
      ...
      def record(self):
            lista = [self.m, self.wall, self.h, self.v, self.w, self.x, self.y, self.z, self.e, self.alpha, self.beta,
                     self.memory, self.message, self.messageid, self.settingm, self.mutationpercentage,
                     self.temperature]
            ...
            while a < len(dna) and self.e > 0.02 * 1.4 ** a:
                  ...
                  if len(gen):
                        if gen[0] == 1:
                              ...
                              #  print(gen,self.cid2)
                              dict = [self.v, self.h, self.m, self.e, self.x, self.y, self.messageid, self.messagekey,
                                      self.message, self.settingm, self.stateofaply, self.wall, self.w, self.z,
...
def dome(n_threads):
      ...
      while True:
            ...
            if airt >= maxairt or airt < 0:
                  factor1 *= -1
            listofxsyszs, listnew = aplythread1(listcells, ep, alpha, beta, xmax, ymax, zmax, cl, a / 3 - a % 3, airt,
                                                watert, aplydna=a % 3 == 0)
            #
...
for li in listcells:
    if li.z>=0:
       ...
       listwater.append(li)
while True:
      iu=int(input("press zero to exit 1 to living cells , 2 to all cells , 3 to records,4 to air cells,5 to water cells: "))
      if iu==0:
          break
      elif iu==1:
            print("from cells ",len(listcells))
            c=listcells[int(input("chose cell to show : "))]
            print("e=",c.e,"beta=",c.beta," Alpha=",c.alpha,"Temperature = ",c.temperature,"x=",c.x,"y=",c.y,"z= ",c.z,"v=",c.v,"h=",c.h,"w= ",c.w,"wall= ",c.wall,"m=",c.m)
            print("dna : ",c.dna,"\ncell id :",c.cid2,"    ")
      elif iu==2:
            print("from cells ",len(listall))
            id2=float(input("chose cell to show : "))
            c=0
            for i in listall:
              if i.cid2==id2:
                c=i
                break
            #c=listall[int(input("chose cell to show : "))]
            print("e=",c.e,"beta= ",c.beta,"Alpha=",c.alpha,"Temperature = ",c.temperature,"x=",c.x,"y=",c.y,"z= ",c.z,"v=",c.v,"h=",c.h,"w= ",c.w,"wall= ",c.wall,"m=",c.m)
            print("dna : ",c.dna,"\ncell id :",c.cid2,"    ")
      elif iu==3 :
            print("from records ",len(listall))
            id2=float(input("chose record to show : "))
            c=0
            for i in listall:
              if i.cid2==id2:
                c=i
                break
            print("alive =" ,c.alive,"death reason = ",c.deathreason,"parents(generation)= ",len(c.parents),"sons = ",len(c.sons))
            print("parents : ",c.parents)
            print("sons : ",c.sons)
            print(" atributes : "+str(c))
      elif iu==4:
            print("from cells ",len(listair))
            c=listair[int(input("chose cell to show : "))] 
            print("e=",c.e,"beta=",c.beta," Alpha=",c.alpha,"Temperature = ",c.temperature,"x=",c.x,"y=",c.y,"z= ",c.z,"v=",c.v,"h=",c.h,"w= ",c.w,"wall= ",c.wall,"m=",c.m)
            print("dna : ",c.dna,"\ncell id :",c.cid2,"    ")
      elif iu==5:
            print("from cells ",len(listwater))
            c=listwater[int(input("chose cell to show : "))] 
            print("e=",c.e,"beta=",c.beta," Alpha=",c.alpha,"Temperature = ",c.temperature,"x=",c.x,"y=",c.y,"z= ",c.z,"v=",c.v,"h=",c.h,"w= ",c.w,"wall= ",c.wall,"m=",c.m)
            print("dna : ",c.dna,"\ncell id :",c.cid2,"    ")